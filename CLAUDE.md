...
Prebuilt components: `packages/server/src/prebuilt/`

## Versioning

When tagging a release, keep these two files in sync:
- `external_dashboards/config.yaml` — `version:` field (used by HA Supervisor)
- `packages/server/package.json` — `version:` field (shown in the Diagnostics page)

Both should match the git tag (e.g. `v0.1.4` → `"0.1.4"`).
...
pnpm dev                        # Dev mode: all packages in parallel (Vite + tsx watch)
...
- Display main chunk is small; Handlebars and uPlot are separate chunks
